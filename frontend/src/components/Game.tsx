...
const Game: React.FC = () => {
    ...
    const startTimer = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/v1/game/startTime",
          {
            method: "POST",
            headers: {
  ...
  const EndTimer = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/v1/game/endTime",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const checkwinnner = async () => {
    if (count === state.game.characters.length) {
      ...
      if (!token) {
        ...
        try {
          const response = await fetch(
            "http://localhost:5000/api/v1/game/addOrUpdateTime",
            {
              method: "POST",
              headers: {
                "Content-type": "application/JSON",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/api/v1/game/createUser",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
